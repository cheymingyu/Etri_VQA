...
from my_gqa_dataset_entry import GQATorchDataset
...
class DeBERTaEncoder(torch.nn.Module):
    def __init__(self):
        ...
        self.projection = torch.nn.Linear(self.deberta.config.hidden_size, 512)

    def forward(self, input_ids, attention_mask):
        outputs = self.deberta(input_ids=input_ids, attention_mask=attention_mask)
        pooled_output = outputs.last_hidden_state  # [CLS] 토큰의 출력
...
class GroundTruth_SceneGraph_Encoder(torch.nn.Module):
    def __init__(self):
        ...
        from my_gqa_dataset_entry import GQA_gt_sg_feature_lookup
...
class PipelineModel(torch.nn.Module):
    def __init__(self):
        ...
        self.question_encoder = DeBERTaEncoder()
    ...
    def forward(self,
        ...
        questions_encoded = self.question_encoder(questions, questions_mask)
