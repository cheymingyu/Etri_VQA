...
class gat_seq(torch.nn.Module):
    ...
    def __init__(self, in_channels, out_channels, edge_attr_dim, ins_dim, num_ins,
        ...
        self.convs = torch.nn.ModuleList(
            [gat(in_channels=in_channels + ins_dim, out_channels=out_channels,  # input is h and ins concat
                 edge_in_channels=edge_attr_dim + ins_dim,  # edge feature is edge_attr and instruction concat
                 heads=gat_heads, concat=False, negative_slope=gat_negative_slope, dropout=dropout, bias=gat_bias) for _
             in range(num_ins)])
    ...
    def forward(self, x, edge_index, edge_attr, instr_vectors, batch):
        ...
        for i in range(num_conv_layers):
            ...
            edge_cat = torch.cat((edge_attr, repeated_ins_edge.to(edge_attr.device)),
                                 dim=-1)  # shape: num_edges X  encode_dim+instruction_dim

            repeated_ins_node = ins[batch]  # pick correct batched instruction for each node
            x_cat = torch.cat((h, repeated_ins_node),
                              dim=-1)  # concat the previous layer node hidden rep with the instruction vector
