...
class GQATorchDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        question_text_tokenized_temp = tokenizer(question_text, return_tensors='pt')
        question_text_tokenized = question_text_tokenized_temp['input_ids'].view(question_text_tokenized_temp['input_ids'].shape[1])
        question_text_tokenized = torch.cat((question_text_tokenized, torch.full((30 - len(question_text_tokenized), ), 2)))
        question_text_mask = question_text_tokenized_temp['attention_mask'].view(question_text_tokenized_temp['attention_mask'].shape[1])
        question_text_mask = torch.cat((question_text_mask, torch.full((30 - len(question_text_mask), ), 0)))

        full_answer_text_tokenized_temp = tokenizer(full_answer_text, return_tensors='pt')
        full_answer_text_tokenized = full_answer_text_tokenized_temp['input_ids'].view(full_answer_text_tokenized_temp['input_ids'].shape[1])
        full_answer_text_tokenized = torch.cat((full_answer_text_tokenized, torch.full((30 - len(full_answer_text_tokenized), ), 2)))
        full_answer_text_mask = full_answer_text_tokenized_temp['attention_mask'].view(full_answer_text_tokenized_temp['attention_mask'].shape[1])
        full_answer_text_mask = torch.cat((full_answer_text_mask, torch.full((30 - len(full_answer_text_mask), ), 0)))
        ...
        return (
            ...
            full_answer_text_tokenized, short_answer_label, types, question_text_mask, full_answer_text_mask
        )
        # return (
        #     questionID, question_text_tokenized, sg_datum, program_text_tokenized,
...
def GQATorchDataset_collate_fn(data):
    ...
    questionID, question_text_tokenized, sg_datum, \
        program_text_tokenized, full_answer_text_tokenized, short_answer_label, types, question_mask, full_answer_mask \
        = zip(*data)
    # questionID, question_text_tokenized, sg_datum, \
    ...
    max_length = max(len(tensor) for tensor in question_text_tokenized)
    question_text_processed = [torch.cat((tensor, torch.zeros(max_length - len(tensor), dtype=tensor.dtype))) for tensor in
                    question_text_tokenized]
    question_text_processed = torch.stack(question_text_processed)
    question_mask_temp = [torch.cat((tensor, torch.zeros(max_length - len(tensor), dtype=tensor.dtype))) for tensor in
                    question_mask]
    question_mask_processed = torch.stack(question_mask_temp)
    ...
    max_length = max(len(tensor) for tensor in full_answer_text_tokenized)
    full_answer_text_tokenized_processed = [
        torch.cat((tensor, torch.zeros(max_length - len(tensor), dtype=tensor.dtype))) for tensor
        in
        full_answer_text_tokenized]
    full_answer_text_tokenized_processed = torch.stack(full_answer_text_tokenized_processed)
    full_answer_mask_temp = [torch.cat((tensor, torch.zeros(max_length - len(tensor), dtype=tensor.dtype))) for tensor in
                          full_answer_mask]
    full_answer_processed = torch.stack(full_answer_mask_temp)
    ...
    return (
        ...
        full_answer_text_tokenized_processed, short_answer_label, types, question_mask_processed, full_answer_processed
    )
